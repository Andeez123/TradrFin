...
const Technicals = () => {
  ...
  useEffect(() => {
    const fetchMarketData = async () => {
      try {
        const response = await fetch('https://cursor-hackathon-virid.vercel.app/api/market-data');
        if (!response.ok) {
          ...
          {([...new Set(dbData.map(item => item.ticker))].slice(0, 4).length > 0
            ? [...new Set(dbData.map(item => item.ticker))].slice(0, 4).map(ticker => (
              <button
                ...
                className={`px-6 py-2 rounded-lg text-sm font-medium transition-all ${activeTicker === ticker
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <div className="flex justify-between items-center mb-4">
              ...
              <div className={`px-2 py-1 rounded text-xs font-bold ${(latestData?.rsi || 50) > 70 ? 'bg-fintech-bear/20 text-fintech-bear' :
