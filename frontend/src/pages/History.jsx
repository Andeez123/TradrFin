...
const History = () => {
    ...
    useEffect(() => {
        const fetchTransactionData = async () => {
            try {
                const response = await fetch('https://cursor-hackathon-virid.vercel.app/api/transactions');
                if (!response.ok) {
                    ...
                    <div className="flex bg-fintech-panel p-1 rounded-lg border border-fintech-border w-full lg:w-auto">
                        {['ALL', 'BUY', 'SELL'].map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={`flex-1 lg:flex-none px-4 lg:px-6 py-2 lg:py-2.5 rounded-md text-xs lg:text-sm font-medium transition-all ${filter === f
                        ...
                        >
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 lg:gap-6">
                                ...
                                <div className="flex items-center gap-3 lg:gap-4 flex-shrink-0">
                                    <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center ${action === 'BUY' ? 'bg-fintech-bull/10 text-fintech-bull' :
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-2 lg:gap-3 mb-1">
                                            ...
                                            <span className={`px-2 py-1 rounded-md text-xs font-bold border flex-shrink-0 ${action === 'BUY' ? 'border-fintech-bull text-fintech-bull bg-fintech-bull/10' :
