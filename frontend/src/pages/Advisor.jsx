...
const Advisor = () => {
    ...
    const handleSend = async (text = input) => {
        ...
        try {
            // 2. Call Backend
            const response = await fetch('https://cursor-hackathon-virid.vercel.app/api/chat', {
                method: 'POST',
            ...
            <div className="flex-1 overflow-y-auto space-y-6 pr-4 custom-scrollbar bg-fintech-card/30 rounded-2xl p-6 border border-fintech-border relative">
                ...
                {messages.map((msg, idx) => (
                    <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                        ...
                        {/* Avatar */}
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-lg ${msg.role === 'ai'
                        ...
                        {/* Bubble */}
                        <div className={`p-4 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-md ${msg.role === 'ai'
